...
const (
	...
	defaultConfigDir         = "/zspace/applications/services/zfileshare/"
	defaultConfigFile        = "zfileshare.json"
	defaultLogFile           = "zfileshare.log"
...
func init() {
	...
	rootCmd.SetVersionTemplate(`{{printf "zfileshare "}}{{printf "%s" .Version}}
`)
}
...
SQLite as data provider.`)
	...
	cmd.Flags().StringVar(&configFile, configFileFlag, viper.GetString(configFileKey),
		`Path to zfileshare configuration file.`)
...
be removed after a successful load.
`)
